            ...
            for (Hero currentHero : heroes) {
                if (currentHero.isAlive()) {
                    if (!(currentHero instanceof Rogue)) {
                        currentHero.incrementMana(5);
                    }
                    currentHero.decrementCooldown(); // Decrement the cooldown for abilities

                    if (currentHero instanceof Warrior && shieldBlockDuration > 0) {
                        shieldBlockDuration--;
                        if (shieldBlockDuration == 0) {
                            Warrior warrior = (Warrior) currentHero;
                            warrior.setDefense(warrior.getInitialDefense());// Reset defense to initial value
                            warrior.desactivateShieldBlock();
                        }
                    }
        ...
        private void handleClericTurn(Cleric cleric, Hero target, boolean useAbility) {
            if (useAbility && cleric.canUseAbility()) {
                ...
                if (attackTarget != null) {
                    ...
                    attackHero(cleric, attackTarget);
                }
            } else {
